...
class FileOp:
    ...
    This class perform three tasks:
        1. Make file detail in specific format and send it to the tracker
    ...
    def send_file(self, start: int, end: int):
        """Send the file from start to end in
           the specified size (chunks) to the conn object.
    ...
    @classmethod
    def receive_file(cls, fname: str, clnt, start: int, sha_list: list):
        """Recieve the file in chunks from different peers
           and append it to the new created file.

        Args:
            fname (str): Name of the file
            clnt : Peer client object
...
class MyClient:
    ...
    def send_msg(self, bin_msg: bytes):
